...
import { forgetPasswordTemplate } from "../template.js";
  ...
  if (role === "recruiter") {
    ...
    const [user] =
      await sql`INSERT INTO users (name, email, password, phone_number, role, bio, resume, resume_public_id) VALUES 
        (${name}, ${email}, ${hasPassword}, ${phoneNumber}, ${role}, ${bio}, ${data.url}, ${data.public_id}) RETURNING user_id, name, email,
         phone_number, role, bio, resume, created_at`;
...
export const loginUser = TryCatch(async (req, res, next) => {
  ...
  if (!email || !password) {
    throw new ErrorHandler(400, "Missing email or password");
  }
...
export const forgotPassword = TryCatch(async (req, res, next) => {
  ...
  const message = {
    to: email,
    subject: "RESET Your Password -hireFast",
    html: forgetPasswordTemplate(resetLink),
  };

  await publishToTopic("send-mail", message).catch(err=>console.log('❌ Failed to send email message'));
